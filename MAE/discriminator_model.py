...
class InpaintGenerator_5(BaseNetwork):
    def __init__(self, residual_blocks=8, init_weights=True):
        ...
        self.encoder = nn.Sequential(
            ...
            nn.Conv2d(in_channels=8, out_channels=64, kernel_size=7, padding=0),
            # nn.InstanceNorm2d(64, track_running_stats=False),
            nn.ReLU(False),
        ...
        self.decoder = nn.Sequential(
            ...
            nn.ReflectionPad2d(3),
            nn.Conv2d(in_channels=64, out_channels=3, kernel_size=7, padding=0),  #1,3,248,248
        )
...
class Discriminator(BaseNetwork):
    ...
    def forward(self, x):
        ...
        outputs = conv5
        if self.use_sigmoid:
            outputs = torch.sigmoid(conv5)  # 后面使用bceloss，需要输入在0-1之间的  所以这里加sigmoid
...
if __name__=='__main__':
    ...
    # y1,y2=edge1(x)
    # print(y1.shape,y2.shape)    #torch.Size([2, 1, 256, 256]) torch.Size([2, 1, 256, 256])
    # dis_input_real = torch.cat((y1, y2), dim=1)
    # print(dis_input_real.shape)  #torch.Size([2, 2, 256, 256])
    # dis1=Discriminator(2)  # inchannels
    # dis_real, dis_real_feat = dis1(dis_input_real)
    # print(dis_real.shape)  #torch.Size([2, 1, 30, 30])
    # print("-----------------------------")
    # for i in dis_real_feat:
    #     print(i.shape)
    ...
    predict=torch.zeros((2,1,30,30))
    target=torch.ones((2,1,30,30))

    # 写法一：按照师兄代码来
    # adverloss=AdversarialLoss(type='nsgan')
    # loss=adverloss(predict,True)

    # 写法二：直接用函数来写，注意一定要实例化loss类
    loss=nn.BCELoss()
    losss=loss(predict,target)
    print(losss)  # 得到的仅仅是一个数字而已  我觉得这个整个部分可以直接拿师兄的啊

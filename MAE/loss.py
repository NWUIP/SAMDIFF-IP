...
from MAE.common import VGG19,gaussian_blur
...
class AdversarialLoss(nn.Module):
    ...
    def __init__(self, type='nsgan', target_real_label=1.0, target_fake_label=0.0):
        ...
        if type == 'nsgan':
            self.criterion = nn.BCEWithLogitsLoss()  # 用的是这个
            # self.criterion = nn.BCELoss()  # 用的是这个
    ...
    def __call__(self, outputs, is_real, is_disc=None):
        if self.type == 'hinge':
            ...
            labels = (self.real_label if is_real else self.fake_label).expand_as(outputs).to("cuda:1")
...
class smgan():
    ...
    def __call__(self, netD, fake, real, masks):  # 判别器、生成图、真实图、masks
        ...
        fakeclone=fake.clone()
        g_fake, _ = netD(fakeclone)  # 对生成图的判断结果
        ...
        if h != ht or w != wt:  # 检查判别器输出和掩码的尺寸是否一致，如果不一致，则使用双线性插值调整它们的大小。
        ...
        d_fake_label = gaussian_blur(masks, (self.ksize, self.ksize), (10, 10)).detach().to("cuda:1")
        d_real_label = torch.zeros_like(d_real).to("cuda:1")  # 0
        g_fake_label = torch.ones_like(g_fake).to("cuda:1")  # 1
        # 想让判别器判断生成图为真

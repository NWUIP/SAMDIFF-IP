...
from guided_diffusion.common import VGG19,gaussian_blur
...
class AdversarialLoss(nn.Module):
    ...
    def __call__(self, outputs, is_real, is_disc=None):
        if self.type == 'hinge':
            ...
            labels = (self.real_label if is_real else self.fake_label).expand_as(outputs).to("cuda:0")
...
class smgan():
    ...
    def __call__(self, netD, fake, real, masks):
        ...
        d_fake_label = gaussian_blur(masks, (self.ksize, self.ksize), (10, 10)).detach().to("cuda:0")
        d_real_label = torch.zeros_like(d_real).to("cuda:0")
        g_fake_label = torch.ones_like(g_fake).to("cuda:0")
